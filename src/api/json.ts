...
jsonRouter.get('/', async (req, res) => {
  try {
    ...
    let remark = (req.query.remark as string) || ''
    if (typeof remark !== 'string') {
      res.status(400)
      res.json({ error: 'remark must be a string' })
      return
    }
    ...
    if (locale in allLocales) {
      fakers.push(
        new Faker({
          locale: allLocales[locale as keyof typeof allLocales],
          seed: seedHash,
        }),
      )
    }
    if (locale.startsWith('zh')) {
      fakers.push(fakerZH_CN)
      fakers.push(fakerZH_TW)
    }
    fakers.push(fakerEN)
    fakers.push(fakerBASE)
    ...
    function generateObject() {
      ...
      for (let field of fields) {
        result[field] = generateData(fakers, field)
      }
      return result
    }
    ...
    try {
      switch (field) {
        ...
        case 'httpStatusMessage':
          return (
            httpStatus[faker.number.int({ min: 100, max: 599 }) as 200] || 500
          )
